...
Built-ins are currently limited to core categories: types, aggregates, numbers (`abs`, `round`, `ceil`, `floor`, `numbers.range`), bits (`bits.and`/`or`/`xor`/`negate`/`lsh`/`rsh`), glob (`glob.match`, `glob.quote_meta`), regex (`regex.match`, `regex.is_valid`, `regex.split`, `regex.find_n`, `regex.replace`), encoding (`json.marshal`, `json.unmarshal`, `json.is_valid`, `base64` encode/decode/is_valid, `base64url` encode/decode/`encode_no_pad`, `hex` encode/decode, `urlquery` encode/decode/`encode_object`/`decode_object`), JSON paths (`json.filter`, `json.remove`), strings (including `replace`, `strings.replace_n`, `trim_prefix`/`trim_suffix`, `strings.reverse`/`count`, `indexof_n`, `strings.any_prefix_match`/`any_suffix_match`), collections (including `object.get`/`keys`/`remove`/`union`/`union_n`/`filter`), crypto (`crypto.md5`/`sha1`/`sha256`, `crypto.hmac.md5`/`sha1`/`sha256`/`sha512`, `crypto.hmac.equal`, `crypto.parse_private_keys`, `crypto.x509.parse_rsa_private_key`), net (`net.cidr_contains`, `net.cidr_intersects`, `net.cidr_is_valid`), semver (`semver.is_valid`, `semver.compare`), time (`time.now_ns`, `time.parse_rfc3339_ns`, `time.parse_ns`, `time.parse_duration_ns`, `time.date`, `time.clock`, `time.weekday`, `time.diff`, `time.add_date`, `time.format` — Go reference-time layouts, IANA zones via the pinned tzinfo-data database), uuid (`uuid.parse`, `uuid.rfc4122` — a random RFC 4122 v4 UUID memoized per key within an evaluation), uri (`uri.parse`, `uri.is_valid` — a byte-exact port of Go's `net/url.Parse`), and comparisons. See the builtins registry for the current list.
...
The private-key parsers are OpenSSL-backed and faithfully reproduce OPA's (Go `crypto/x509`) accept/reject behavior, including its supported-key-type set: RSA, Ed25519, X25519, and EC on Go's four `crypto/elliptic` curves (P-224/256/384/521). DSA, Ed448, X448 and other EC curves are unsupported and yield undefined (never an exception). `crypto.parse_private_keys(pem)` scans every PEM block (only `-----BEGIN` at a line start counts, matching Go's `pem.Decode`), parsing the `RSA PRIVATE KEY`/`PRIVATE KEY`/`EC PRIVATE KEY` types (PKCS#1/PKCS#8/SEC1) and silently skipping any other block (e.g. a certificate); each key becomes its Go `encoding/json` shape — `rsa.PrivateKey` / `ecdsa.PrivateKey` objects (big-integer fields), the std-base64 of a 64-byte Ed25519 key, or `{}` for an X25519 (`ecdh.PrivateKey`) key. An empty string is `null`, a string with no key blocks is `[]`, and a recognized-but-malformed or unsupported-type block is undefined. `crypto.x509.parse_rsa_private_key(s)` returns the first key as a JWK (RSA, EC, or `OKP`/Ed25519/X25519 — the `rsa` in the name is historical); it accepts PEM or a std-base64-encoded DER (which, being un-armored after decoding, yields `null`), an empty string is undefined, and JWK members are base64url of the minimal big-endian integer (RSA) or fixed curve-width coordinate (EC/OKP). The JWK encoding omits P-224 (no JOSE registration), so a P-224 key is a struct under `parse_private_keys` but undefined under `parse_rsa_private_key` — matching OPA. Both follow Go's per-format trailing-data rule (parsing reads one leading ASN.1 SEQUENCE: PKCS#8 and SEC1 ignore trailing bytes, PKCS#1 rejects them). One DoS-guard divergence: a block whose leading ASN.1 element exceeds 64 KiB is treated as undefined without invoking OpenSSL (Ruby's `OpenSSL::PKey.read` trial-parses every format and would spend seconds on a multi-megabyte garbage DER, where Go's typed x509 parsers fast-fail). 64 KiB is far above any real private key (an RSA-16384 DER is ~9 KB; only an absurd >130K-bit key would exceed it), so the result matches OPA for every genuine key.

The `json.filter`/`json.remove` built-ins take an object document and an array or set of paths. A path is either a `/`-separated string with JSON-pointer escaping (`~1` for `/`, `~0` for `~`) or an array of literal segments; a numeric string segment selects an array index (canonical form only — no leading zeros, matching OPA). A leading run of slashes in a string path is stripped before splitting (so `/a/b` and `a/b` are equivalent, matching OPA's `parsePath`), and the empty string is an empty path. `json.filter(doc, paths)` projects only the listed paths: a terminal path keeps the whole subtree, a path that descends past a scalar keeps the scalar, and a path into a missing/empty branch leaves an empty container. `json.remove(doc, paths)` drops the listed paths; removing array elements reindexes the array, and when several indices under one array are removed they are taken against the original positions. Overlapping paths merge (a terminal path wins regardless of order). A non-object document, a paths argument that is neither an array nor a set, or a path element that is neither a string nor an array yields undefined.
...
- `time.parse_ns` / `time.format` resolve a named zone *abbreviation* in the parsed value (e.g. `MST`, `PST`, `GMT`) to a **zero** UTC offset. Go's `time.Parse` resolves abbreviations against the process-local timezone, which makes the result host-dependent (and so non-deterministic across deployments); the gem deterministically treats every abbreviation as offset 0, which is what `TZ=UTC opa eval` yields. Explicit numeric offsets (`-0700`, `+05:30`) and `Z` are unaffected and shift the instant exactly as OPA does. The tz-aware time builtins also share a far-future (post-~2100) DST-projection divergence from OPA's bundled tzdata, since they use the pinned tzinfo-data database.
- `uri.parse` percent-decodes path/host/fragment to raw bytes (matching Go's byte-oriented `net/url` and the gem's `urlquery.decode`). A `%`-sequence that decodes to invalid UTF-8 (e.g. `%FF`) therefore yields the raw byte — the same value OPA holds — but, like every non-UTF-8 string the gem can construct (see the deferred string-encoding normalization), OPA renders it as U+FFFD in JSON output whereas Ruby's `JSON.generate` raises. Reachable only via a deliberately malformed URI; valid-UTF-8 percent-encoding (e.g. `%C3%A9` → `é`) is byte-exact.
