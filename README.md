...
The `json.filter`/`json.remove` built-ins take an object document and an array of paths. A path is either a `/`-separated string with JSON-pointer escaping (`~1` for `/`, `~0` for `~`) or an array of literal segments; a numeric string segment selects an array index (canonical form only — no leading zeros, matching OPA). `json.filter(doc, paths)` projects only the listed paths: a terminal path keeps the whole subtree, a path that descends past a scalar keeps the scalar, and a path into a missing/empty branch leaves an empty container. `json.remove(doc, paths)` drops the listed paths; removing array elements reindexes the array, and when several indices under one array are removed they are taken against the original positions. Overlapping paths merge (a terminal path wins regardless of order). A non-object document, a non-array paths argument, or a path element that is neither a string nor an array yields undefined.
