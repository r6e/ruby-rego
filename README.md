...
- `glob.match` implements correct glob semantics rather than reproducing known bugs in OPA's matcher (gobwas/glob). Unlike OPA: character classes may use multiple ranges such as `[A-Za-z]` ([gobwas #47](https://github.com/gobwas/glob/issues/47)); `?` and `[...]` match non-ASCII characters by Unicode codepoint ([gobwas #41](https://github.com/gobwas/glob/issues/41)); and `?`/`[!...]` require exactly one character instead of also matching the empty string. Well-formed ASCII patterns behave identically to OPA. Malformed patterns yield an undefined result.
